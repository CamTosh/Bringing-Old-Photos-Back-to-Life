...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_folder",
        type=str,
        default="/Users/camille.t/Documents/perso/colorize-image/photo_restoration/output",
    ...
    if not opts.with_scratch:
        stage_1_command = (
            "/usr/local/bin/python3.8 test.py --test_mode Full --Quality_restore --test_input "
            + stage_1_input_dir
            + " --outputs_dir "
            + stage_1_output_dir
            + " --gpu_ids "
        ...
        stage_1_command_1 = (
            "/usr/local/bin/python3.8 detection.py --test_path "
            + stage_1_input_dir
            + " --output_dir "
            + mask_dir
        ...
        stage_1_command_2 = (
            "/usr/local/bin/python3.8 test.py --Scratch_and_Quality_restore --test_input "
            + new_input
            + " --test_mask "
            + new_mask
            + " --outputs_dir "
    ...
    stage_2_command = (
        "/usr/local/bin/python3.8 detect_all_dlib.py --url " + stage_2_input_dir + " --save_url " + stage_2_output_dir
    )
    ...
    stage_3_command = (
        "/usr/local/bin/python3.8 test_face.py --old_face_folder "
        + stage_3_input_face
    ...
    if not os.path.exists(stage_4_output_dir):
        os.makedirs(stage_4_output_dir)
    stage_4_command = (
        "/usr/local/bin/python3.8 align_warp_back_multiple_dlib.py --origin_url "
